...
namespace FoheartMC
{
    public class FoheartModel : MonoBehaviour
    {
        ...
        void loadBoneMaps()
        {
            try
            {
                XDocument doc = XDocument.Load(/*"E:/UnityRelease/" +*/ ConfigName);
        ...
        public virtual void applyBoneRotations(ActorFrameData data)
        {
            ...
            foreach (var BoneR in data.boneRotQuat)
            {
                ...
                if (BoneT)
                {
                    ...
                    if (BoneR.Key == 0)
                    {
                        Quaternion convQuatApply = new Quaternion(-BV.y, -BV.x, -BV.z, BV.w);
                    ...
                    {
                        Quaternion convQuatApply = new Quaternion(BV.z, BV.y, -BV.x, BV.w);
                    ...
                    if (BoneR.Key == 7)
                    {
                        ...
                        Quaternion rot = Quaternion.Euler(0.0f, 90.0f, 180.0f);
                        BoneT.localRotation = rot * convQuatApply;
                    }
                    else if (BoneR.Key == 8)
                    {
                    ...
                    {
                        Quaternion convQuatApply = new Quaternion(-BV.z, BV.y, BV.x, BV.w);
                        Quaternion rot = Quaternion.Euler(0, -180.0f, 0);
                        BoneT.localRotation = rot * convQuatApply;
                    }
